...
function Home() {
    const [searchTerm, setSearchTerm] = useState('');
    const [searchType, setSearchType] = useState('Songs'); // Define searchType state
    ...
    const handleSearchTypeChange = (type) => { // Define handleSearchTypeChange
        setSearchType(type);
    };
    ...
    const handleSearchSubmit = async (event) => {
        event.preventDefault();
        try {
            const response = await axios.get(`http://localhost:4000/api/songs?name=${searchTerm}`);
    ...
    const renderSearchResults = () => { // Define renderSearchResults
        return searchResults.map((result, index) => (
            <div key={index}>
